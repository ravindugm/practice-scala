...
object HOFsCurries extends App {
  ...
  def compose[A, B, T](f: A => B, g: T => A): T => B =  // x is T, g of T return A, f of A return B then T all the way to B
    x => f(g(x))

  def andThen[A, B, C](f: A => B, g: B => C): A => C =  // x is A, f of A is return B, g of B return C then A all the way to C
    x => g(f(x))
